...
if (isset($_GET['postId'])) {
    ...
    $sql = "SELECT * FROM books WHERE bookId = $bookId AND userId = $userId";
    $query = mysqli_query($conn, $sql);
...
if (isset($_POST["new_post"])) {
    if ($_FILES["image_upload"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($temp_name, $target_path)) {
            ...
            $insertQuery = "INSERT INTO books(Name, Favorite, Status, Score, EpisodeProgress, Notes, userId, file_name) 
...
if (isset($_REQUEST['update'])) {
    ...
    if ($_FILES["new_image_upload"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($temp_name, $target_path)) {
            $updateQuery = "UPDATE books SET Name = '$name', Favorite = $favorite, Status = '$status', Score = $score, EpisodeProgress = $episodeProgress, Notes = '$notes', file_name = '$target_path' WHERE bookId = $bookId";
